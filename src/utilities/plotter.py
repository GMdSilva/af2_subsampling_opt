...
class Plotter:
    ...
    def plot_mut_analysis(self, x_values: Dict, mut_list: Dict) -> None:
        ...
        if x_values == 'ground_pop_diff' or 'ground_pop_test':
            colors = ['red' if effect == '-' else ('grey' if effect == "ref" else 'blue') for effect in effects_ground]
        else:
            colors = ['red' if effect == '-' else ('grey' if effect == "ref" else 'blue') for effect in effects_alt1]

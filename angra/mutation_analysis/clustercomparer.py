...
from user_settings.config import SYSTEM_NAME, IS_JUPYTER, PREDICTION_ROOT, MUTANT_DATA
...
class ClusterComparer(MutantStateFinder):
    ...
    def find_optimal_clusters(self, wildtype_results: np.ndarray,
    ...
    def measure_mutation_effects(self,
                                 kmeans,
                                 clusters_wildtype,
                                 measure_accuracy: bool = True) -> None:
        """
        ...
        self.clustering_results = {'all_rmsd_measurements': all_rmsd_measurements,
    ...
    def build_report(self, wt_evaluation: Union[List[str], Tuple[str, ...]],
        ...
        self.clustering_difference_results = {'states': states,
                                   'diff_results': diff_results,
                                   'sorted_mut_data': sorted_mut_data}
        report_dict = {
    ...
    def f_plot_clustering_diffs(self):
        diff_results = self.clustering_difference_results['diff_results']
        states = self.clustering_difference_results['states']
        sorted_mut_data = self.clustering_difference_results['sorted_mut_data']
    ...
    def f_plot_cluster_results(self) -> None:
        """
        ...
        all_rmsd_measurements = self.clustering_results['all_rmsd_measurements']
        results = self.clustering_results['clustering_results']

        cluster_colors = results['cluster_colors']
        annotations = results['cluster_annotations']

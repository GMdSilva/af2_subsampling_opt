"""Analyzes modes/states obtained from each subsampling parameter set"""
...
def get_silverman_bandwidth(data: Union[List[float], np.ndarray]) -> float:
    ...
    Args:
        data (Union[List[float], np.ndarray]): The dataset for which to calculate the bandwidth.
...
def find_modalities(density: Union[List[float], np.ndarray],
                    ...
                    quantile: float = 30) -> Tuple[int, np.ndarray, np.ndarray, np.ndarray, float]:
    """
    ...
    Args:
        ...
        quantile (float, optional): Quantile used for setting the threshold. Defaults to 30.
    ...
    """
    threshold = np.percentile(density, quantile)
...
def sort_peaks_by_population(mode_data: Dict[str, Union[np.ndarray, List]])\
    ...
    sorted_indices = np.argsort(populations)

    return populations[sorted_indices], x_values[peaks][sorted_indices], modes[sorted_indices]


def analyze_modes(data: Union[List[float], np.ndarray],
                  rmsd_range: Union[List[float], np.ndarray],
                  trial: int,
                  bandwidth: float = 0.1,
                  quantile: float = 30) -> Dict[str, Union[int, float, List[float]]]:
    """
    ...
    Args:
        ...
        trial (int): Identifier for the trial.
    ...
    Returns:
        dict: With keys 'num_modes', 'populations', 'modes', 'peaks', 'distribution', and 'trial'.
    """
    x_values, density = estimate_density(data, bandwidth)
    num_modes, populations, modes, peaks, threshold = find_modalities(density, x_values, quantile)

    plt.rcParams['font.size'] = 18
    plt.close()
    plt.plot(x_values, density, label=trial)
    plt.scatter(x_values[peaks], density[peaks], color='red')  # plot peaks in red
...
def two_state_analysis(mode_data: Dict[str, Union[int, float, List[float]]]) ->\
    ...
    """
    sorted_pops, sorted_peaks, sorted_modes = sort_peaks_by_population(mode_data)
...
def mode_sanity_checks(modality_results: dict, rmsd_range: str) -> bool:
    """
    ...
    print(f"Peak range {rmsd_range} with Alt1 RMSD of {alt1_peak:.3g} A "
          f"for parameter {trial} accepted ")
    return True
